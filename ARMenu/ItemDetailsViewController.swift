...
class ItemDetailsViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if viewARButton.isEnabled {
             btnColor = UIColor(red: 0, green: 0.47, blue: 1, alpha: 1)
        } else {
            btnColor = UIColor(red: 0.60, green: 0.78, blue: 0.99, alpha: 1.00)
        }

        viewARButton.layer.cornerRadius = 5
        viewARButton.layer.backgroundColor = btnColor.cgColor
        viewARButton.setTitleColor(.white, for: .normal)
    }
